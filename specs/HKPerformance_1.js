...
export default function () {
    ...
    // Dinamik bir kullanıcı oluşturmak için rastgele veriler
    const userId = Math.floor(Math.random() * 1000);
    const payload = JSON.stringify({
        username: `user${__VU}_${userId}`, // Kullanıcı adı, her sanal kullanıcı için benzersiz olacak şekilde oluşturulur
    ...
    // GET isteği ile kullanıcı bilgilerini doğrulama
    let getUserRes = http.get(`${BASE_URL}/users/${userId}`, { headers });
    check(getUserRes, {
